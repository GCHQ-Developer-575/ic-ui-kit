...
export const ExternalFiltering = () => {
  ...
  return (
    <IcSelect loading={loading} timeout={1000} debounce={300} label="What is your favourite coffee?" searchable disableFilter options={results} onIcOptionSelect={handleOptionSelect} onIcChange={handleChange} onIcClear={handleClear}/>
  );
};
